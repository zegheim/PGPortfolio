...
class Trader:
    ...
    def _initialize_logging_data_frame(self, initial_BTC):
        ...
        for coin in self._coin_name_list:
            logging_dict[coin] = 0
        self._logging_data_frame = pd.DataFrame(
            logging_dict, index=pd.to_datetime([time.time()], unit="s")
        )
    ...
    def __trade_body(self):
        ...
        omega = self._agent.decide_by_history(
            self.generate_history_matrix(), self._last_omega.copy()
        )
        ...
        logging.info(f"total returns are {(self._total_capital - 1) * 100:.3f}%")
        logging.debug("=" * 30)
