...
class Poloniex:
    def __init__(self, APIKey="", Secret=""):
        ...
        self.float_roundPercent = (
            lambda floatN, decimalP=2: str(round(float(floatN) * 100, decimalP)) + "%"
        )
        ...
        self.marketOrders = lambda pair="all", depth=10: self.api(
            "returnOrderBook", {"currencyPair": pair, "depth": depth}
        )
        self.marketChart = lambda pair, period=day, start=time.time() - (
            week * 1
        ), end=time.time(): self.api(
            "returnChartData",
            {"currencyPair": pair, "period": period, "start": start, "end": end},
        )
        self.marketTradeHist = lambda pair: self.api(
            "returnTradeHistory", {"currencyPair": pair}
        )  # NEEDS TO BE FIXED ON Poloniex
