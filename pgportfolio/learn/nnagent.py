...
class NNAgent:
    def __init__(self, config, restore_dir=None, device="cpu"):
        ...
        self.__future_omega = (self.__future_price * self.__net.output) / tf.reduce_sum(
            self.__future_price * self.__net.output, axis=1
        )[:, None]
    ...
    def __set_loss_function(self):
        ...
        def loss_function7():
            return -tf.reduce_mean(tf.log(self.pv_vector)) + LAMBDA * tf.reduce_mean(
                tf.reduce_sum(
                    -tf.log(1 + 1e-6 - self.__net.output), reduction_indices=[1]
                )
            )
        ...
        def with_last_w():
            return -tf.reduce_mean(
                tf.log(
                    ...
                    - tf.reduce_sum(
                        tf.abs(self.__net.output[:, 1:] - self.__net.previous_w)
    ...
    def init_train(self, learning_rate, decay_steps, decay_rate, training_method):
        learning_rate = tf.train.exponential_decay(
            learning_rate, self.__global_step, decay_steps, decay_rate, staircase=True
        )
