...
class TDAgent(object):
    ...
    def find_bcrp(self, X, max_leverage=1):
        ...
        while True:
            res = minimize(
                objective, x_0, bounds=bnds, constraints=cons, method="slsqp"
            )
            eps = 1e-7
            if (res.x < 0 - eps).any() or (res.x > max_leverage + eps).any():
                ...
                if np.isnan(res.x).any():
                    logging.warning(
                        "Solution does not exist, use uniform portfolio weight vector."
                    )
    ...
    def corn_expert(self, data, w, c):
        ...
        if w == 0:
            ...
            for i in np.arange(w, T):
                ...
                if datacorr >= c:
                    ...
                    histdata[m, :] = data[
                        i - 1, :
                    ]  # minus one to avoid out of bounds issue
        ...
        while True:
            ...
            if (res.x < 0 - eps).any() or (res.x > 1 + eps).any():
                ...
                logging.debug(
                    "Optimal portfolio weight vector not found, trying again..."
                )
                continue
            elif res.success:
                break
