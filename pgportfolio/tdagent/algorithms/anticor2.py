...
class ANTICOR2(TDAgent):
    ...
    def decide_by_history(self, x, last_b):
        ...
        for k in np.arange(1, self.window):
            self.exp_w[k - 1, :] = self.update(
                self.history, self.exp_w[k - 1, :], k + 1
            )
            self.exp_w2[k - 1, :] = self.update(
                self.data_day, self.exp_w2[k - 1, :], k + 1
            )
        ...
        for k in np.arange(1, self.window):
            ...
            self.exp_w2[k - 1, :] *= self.data_day[-1, :] / np.dot(
                self.data_day[-1, :], self.exp_w2[k - 1, :].T
            )
    ...
    def update(self, data, last_b, w):
        ...
        if T >= 2 * w:
            ...
            mCorr = np.zeros((N, N))
            mCorr = np.multiply(mCov, sigma != 0) / np.sqrt(
                np.multiply(sigma, sigma != 0)
            )
